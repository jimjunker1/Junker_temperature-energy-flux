...
-   Abstract word count: 260/350

-   Main text word count (w/ references): 8201

-   Page count w/ references: 34/30

-   Appendices: 2

Open Research Statement: All data and code used in this work are
publicly available at the following link:
<https://github.com/jimjunker1/Junker_temperature-energy-flux>. Upon
acceptance, all data and code will be submitted to relevant repositories
(e.g., Dryad, Zenodo) for archival.

Key words: energy flux; climate change; food webs; species traits;
temperature; environmental filtering
...
globally. While the overall influence of warming on food webs is often
...
stream food webs spanning a natural \~25$^\circ$C temperature gradient
(5-27$^\circ$C). We then related these patterns to species and
community trait distributions of mean body size and population
biomass turnover (*P:B*) within and across ecosystems. We predicted that
...
by smaller body size (\~9% per $^\circ$C) and higher *P:B* (\~7% faster
per $^\circ$C) populations on average. Additionally, organic matter
...
to arise through random sorting of species, suggesting stronger
...
rate, handling time, growth rates, etc.\; @dell2014], to broad-scale
shifts in community assembly and structure [@saito2021; @gibert2019a;
...
traits translate to patterns of energy demand [@loreau2001;
...
reducing community evenness and favoring a few well-adapted species
...
additional processes unrelated to environmental filtering that alter
species relative abundances [e.g., species interactions,
@therriault1999; demographic stochasticity, @hubbell2001]. These
additional processes can exaggerate or counter any skew in trait
...
Body size is a fundamental trait is influenced by temperature and has great potential to influence energy flux [for ectotherms,
@atkinson1994; @gardner2011; @daufresne2009; but see @riemer2018 for
extensive deviations across endotherms]. Reduced body size in response
...
on consumer-resource interactions or food web structure [@gibert2019a;
...
changes suggest that warming should lead to a smaller, faster world in
...
[@junker2020]. We therefore predicted that annual OM fluxes
to consumers would increase with stream temperature, mirroring patterns
in resource availability and consumer energy demand. We also
hypothesized that temperature would act as a principle environmental
...
patterns would not arise by random sorting (i.e., 'non-random ordering'), but more likely through deterministic filtering for species traits. Our results should help refine general predictions about how ongoing
...
[@junker2021]. In each stream, we measured temperature and water depth
...
were part of a separate warming manipulation during the un-manipulated,
...
each stream. Within the sampler, inorganic substrates were disturbed to
...
calculated by the changes in mean body size (*M*) over a given time
...
(mg AFDM) using published length-mass regression cited above, and
...
after 10,000 resamplings, a minimum growth rate of 0.001 was used. To
...
For each interval, size classes were summed for each taxon to calculate
total population-level production. Intervals were summed to estimate
annual secondary production (g AFDM m^-2^ y^-1^).

## Organic Matter Consumption Estimates {#organic-matter-consumption-estimates}

Organic matter fluxes (g AFDM m^-2^ y^-1^) through the community were
...
estimate consumption of organic matter. Consumer diets were quantified
in numerically dominant taxa through direct inspection of gut contents
from multiple individuals throughout the year. Removal and preparation
of gut tracts for quantification followed the methods outlined in
Rosi-Marshall [-@rosi-marshall2016]. To estimate variability in diet
...
multivariate model [@fordyce2011; @coblentz2017]. We estimated trophic redundancy within and across stream food webs by calculating proportional similarities [*PS*, @whittaker1952] among modeled diet estimates. For each food
...
was calculated for each taxon across sampling intervals (typically \~1
...
through a Monte Carlo approach, wherein bootstrapped vectors of
secondary production for each taxon (see *Secondary production* methods
above) were resampled and consumption estimated with the TBP method
using modeled diet proportions (see *Diet analysis* in supporting information, Appendix S1), diet-specific assimilation efficiencies, and
...
0.40-0.50). Beta distributions were fit in R [@rcoreteam2022] using the
...
To visualize and quantify the evenness of OM fluxes among taxa within
...
flux of species *i*, $p_i$, is ordered $p_1 \leq p_2 \leq ... p_S$. The
Lorenz curve shows how a value, in this case OM flux, accumulates with
increasing cumulative proportion of taxa. In a community with an equal
distribution of OM flux among taxa, the Lorenz curve is simply a
...
towards small body size (*M*) and higher *P:B* across and within
...
above) for each taxon within each stream. The mean of all populations
within a stream was calculated and the least squares estimate was
calculated between *log~e~*-transformed $\overline{M}$ or
...
distributed residual variation to allow the use of the ordinary least
squares estimation within the `lm()` function in R.

To quantify how the distribution of organic matter fluxes were modified
*within* a community and whether this modification was related to
...
other parametric or moments-based approaches because it is well defined
without needing to understand the moments of the distribution
...
coefficients exist in the range [-1, 1], where -1 indicates that OM
fluxes are skewed perfectly away from a trait and 1 indicates that
...
relative distribution of OM fluxes within a community. The number of
unique orderings of species increases to computationally intractable
numbers very quickly (e.g., $S!$ for 10 species
$\approx 3.6 \times 10^{6}$ unique orderings). Therefore, we chose to
permute a random subset of each stream community's feasible set by
...
`r round((min_flux$flux_mg_m_y_quant97.5/1000),1)`; mean (95% percentile
interval [PI])) to `r round((max_flux$flux_mg_m_y_mean/1000),1)`
...
Within streams, diet similarity ranged from
...
with a mean similarity of `r round(among_overlap_mean,2)*100`%
(`r round(among_overlap_quant2.5,2)*100`%--`r round(among_overlap_quant97.5,2)*100`%).
Diet similarities based on pairwise comparisons among streams showed
...
relationships, with some streams showing skew toward larger-bodied taxa
(positive skew), others with most OM fluxes through smaller-bodied sizes
(negative skew), and some showed no difference among body sizes (skew
range:
...
stream, where fluxes were heavily skewed toward small-bodied taxa.
...
taxa could be attributed to these traits vs. random community assembly
...
structured OM flux distribution---i.e., favoring fluxes through high
*P:B* taxa--at warmer temperatures. The likelihood of higher relative
fluxes among high *P:B* taxa became much more likely with warming, and
...
@woodward2010]. Here, we demonstrate that along a wide natural gradient
of ecosystem temperatures, warming acts as a strong environmental filter
of species traits. In particular, we found that increasing temperatures
...
addition, we found that warming systematically skewed OM fluxes *within*
...
deviations exists across ecosystems [@thresher2007; @ogorman2012;
@ohlberger2013]. We observed a clear decrease in the average individual
...
experiment conducted in one of our study streams showing a 3$^\circ$C
increase in temperature shifted community biomass and productivity from
smaller to larger organisms [@nelson2017; @nelson2017a]. Moreover, a
study of diatom communities across the same temperature gradient in
Iceland reported no consistent change in body size at either the
population- or community-level [@adams2013; @ogorman2012]. Other studies
...
reduced body size with warming, clearly many processes modify the
direction and magnitude of body size shifts and how they play out from
individual to ecosystem scales [e.g., growth and developmental rates,
resource supply, competition, predation, etc., @ohlberger2013].
...
communities, leading to an increase OM fluxes through populations with
...
short lifespan\; @munch2009; high growth rate\; @donhauser2020; etc.
@stoks2014], especially at relatively high temperatures.

## Further considerations

A growing body of theoretical and empirical research is beginning to explore the implications of warming for the dynamics of ecosystems [@fussmann2014]. ‘Natural laboratories’, such as the geothermal watershed studied here, offer a unique opportunity to isolate the effects of temperature and the dominant processes by which it may modulate ecosystem structure and dynamics [@ogorman2014]. That said, here as in most ecological studies, our observations may have varying applicability across ecosystem types and spatiotemporal contexts. For instance, this study has obvious direct implications for the future of high-latitude ecosystems under a warmer climate and these conditions may extend to other light-driven ecosystems, such as many grassland, urban, or high-altitude environments that exhibit pronounced coupling of light regimes—and thereby energy supply regimes—and the timing of temperature-driven metabolic demands [@mcmeans2015; @huryn2019a]. In the absence of other limiting factors, increasing temperatures may alter the balance of resource supply and consumer demand itself due to different temperature responses of photosynthesis and metabolism [@oconnor2009; @gilbert2014; @brown2004]. This differential response could be exaggerated if light availability limits basal resource supply with potential consequences for the selection of reduced body sizes and life-history traits that lead to faster biomass turnover [@kozlowski2004; @delong2012a]. This dynamic runs counter to ecosystems where food resource/energy regimes are decoupled from environmentally driven consumer energy demands, such as many donor-controlled ecosystems [e.g., headwater streams, @hynes1975; islands, @polis1996a]. The implications of this varied temporal covariance between resource supply and consumer demand [e.g., phenological mismatch, @cushing1977; @renner2018] may have partly explain varied responses to warming at the species, community, and ecosystems levels across ecosystem types [@visser2019].
...
biomass turnover. Further, biomass turnover rate---but not body
...
We are grateful to Sigurður Guðjonsson, Guðni Guðbergsson, and the staff
at the Veiðimlastofnun for providing laboratory space and logistical
support. We are also grateful to Sveinbj€orn Steinþorsson at the
...
(log~e~[g AFDM m^-2^ y^-1^]) by taxon. Taxa abbreviations: F:Naid,
